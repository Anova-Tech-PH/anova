...
import { getRegistrationPageBySlug } from "@/features/registration-pages/queries";
import { filterTicketsByRegistrationPage } from "@/features/registration-pages/filter-tickets";

export default async function RegisterByPageSlugPage({
  params,
  searchParams,
}: {
  params: Promise<{ orgSlug: string; eventSlug: string; pageSlug: string }>;
  ...
  // Look up the registration page by slug
  const registrationPage = await getRegistrationPageBySlug(event.id, pageSlug);
  if (!registrationPage) notFound();
  ...
  const allTicketsWithAvailability = (ticketTypes ?? []).map((t) => ({
    ...t,
    sold: countMap[t.id] ?? 0,
    available: t.quantity ? t.quantity - (countMap[t.id] ?? 0) : null,
  }));

  // Filter tickets based on registration page's ticket_type_ids
  const ticketsWithAvailability = filterTicketsByRegistrationPage(
    allTicketsWithAvailability,
    registrationPage.ticket_type_ids,
  );
    ...
    <div className="mx-auto max-w-lg px-4 py-12">
      ...
      <div className="mb-8 rounded-xl border bg-gradient-to-br from-primary/[0.04] to-transparent p-5">
        ...
        {registrationPage.name && (
          <p className="mt-1 text-sm font-medium text-primary">{registrationPage.name}</p>
        )}
      ...
      {ticketsWithAvailability.length === 0 ? (
        <div className="mt-8 rounded-xl border border-dashed bg-muted/20 py-12 text-center text-muted-foreground">
          No tickets available on this registration page.
        </div>
      ) : (
        <RegistrationFlow
          eventId={event.id}
